...
class ShoppingCartController {
  ...
  //#region getShoppingCartUser [ 查詢一筆購物車資料 ]
  /** 查詢一筆購物車資料 */
  static async getShoppingCartUser(req: Request, res: Response, next: NextFunction) {
    //#region [ swagger說明文件 ]
    /**
     * #swagger.tags = ["ShoppingCart - 購物車資料表 API"]
     * #swagger.description = "查詢一筆購物車資料"
     * #swagger.responses[200] = {
          description: "成功",
          schema: {
            "statusCode": 200,
    ...
    */
    ...
    try {
      ...
      if (!userId) return handleResponse(res, HttpStatusCode.BadRequest, HttpMessage.BadRequest);

      const result = await ShoppingCart.findOne({ user: userId });
  ...
  //#endregion getShoppingCartUser [ 查詢一筆購物車資料 ]

  //#region deleteShoppingCarts [ 刪除所有資料 ]
  /** 刪除所有資料 */
  static async deleteShoppingCarts(_req: Request, res: Response, next: NextFunction) {
    //#region [ swagger說明文件 ]
    /**
     * #swagger.tags = ["ShoppingCart - 購物車資料表 API"]
